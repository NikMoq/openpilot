...
// WayIndex provides fast spatial lookup for OSM ways using a grid
...
// NewWayIndex builds a grid spatial index from ways
...
// MatchedWay holds a way and its distance to a point
type MatchedWay struct {
	Way      Way
	Distance float64 // meters
	Bearing  float64 // degrees 0-360, bearing of nearest segment
}

// FindNearestWays finds all ways within maxDist meters of (lat, lon),
// sorted by distance. Uses 9-cell grid search.
func (wi *WayIndex) FindNearestWays(lat, lon float64, maxDist float64) []MatchedWay {
	...
	maxDistDeg := maxDist / 111000.0 // approximate degrees per meter
	...
	for dLat := -cellRange; dLat <= cellRange; dLat++ {
		for dLon := -cellRange; dLon <= cellRange; dLon++ {
			...
			for _, w := range wi.grid[key] {
				// Hash from first node coordinates for dedup
				nodes := w.Nodes()
				var wayHash uint64
				if len(nodes) > 0 {
...
// distanceAndBearingToWay returns the shortest distance from pos to any segment of way,
// and the bearing of that segment in degrees [0,360).
