...
// cameraTypeMap maps raw Russian strings to normalized types
...
// normalizeCameraType converts raw Russian type string to normalized type
...
func ParseSpeedcam(data []byte) ([]RawCamera, error) {
	...
	for scanner.Scan() {
		...
		// Skip header lines (start with digit|>)
		...
		// Validate coordinates (Russia + Kaliningrad)
	...
	// Deduplicate: same type within 5 meters
	cameras = deduplicateCameras(cameras)

	return cameras, nil
}

// gridCellSize for deduplication (~50m at equator)
const dedupCellSize = 0.0005

// deduplicateCameras removes duplicate cameras of the same type within 5 meters
func deduplicateCameras(cams []RawCamera) []RawCamera {
	...
	for _, cam := range cams {
		latIdx := int(math.Floor(cam.Lat / dedupCellSize))
		lonIdx := int(math.Floor(cam.Lon / dedupCellSize))
