#!/usr/bin/env python3
"""Generate BMFont atlas files (.fnt/.png) from TTF/OTF fonts.

This script uses raylib to rasterize glyphs and generate atlases.
On Windows, rl.export_image() is broken, so we use a custom PNG writer.
"""
...
GLYPH_PADDING = 6
EXTRA_CHARS = "–‌✓×°§•X⚙✕◀▶✔⌫⇧␣○●↳çêüñ–‌✓×°§•€£¥"
EXTRA_GLYPHS = "иэыИЭЫ"  # Additional Cyrillic glyphs for better coverage
UNIFONT_LANGUAGES = {"th", "zh-CHT", "zh-CHS", "ko", "ja"}

# Russian Cyrillic whitelist
RUSSIAN_WHITELIST = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя"
...
def _export_image_png(image, path: Path) -> bool:
  """Export raylib Image to PNG without rl.export_image (broken on Windows).

  Supports R8G8B8A8 (32bpp) and GRAY_ALPHA (16bpp) formats.
  """
  width = image.width
  height = image.height
  ...
  if image.format == rl.PixelFormat.PIXELFORMAT_UNCOMPRESSED_R8G8B8A8:
    bpp = 4
  elif image.format == rl.PixelFormat.PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA:
    bpp = 2
  elif image.format == rl.PixelFormat.PIXELFORMAT_UNCOMPRESSED_R8G8B8:
    bpp = 3
  elif image.format == rl.PixelFormat.PIXELFORMAT_UNCOMPRESSED_GRAYSCALE:
    bpp = 1
  else:
  ...
  raw_data = bytes(rl.ffi.buffer(image.data, data_size))

  # Build PNG raw data: each row starts with filter byte (0 = None)
  row_size = width * bpp
  png_raw = bytearray()
  for y in range(height):
    png_raw.append(0)  # Filter type: None
    row_start = y * row_size
    png_raw.extend(raw_data[row_start:row_start + row_size])

  # Compress with zlib
  compressed = zlib.compress(bytes(png_raw), level=6)

  # Build PNG file
  png_signature = b'\x89PNG\r\n\x1a\n'

  # IHDR chunk
  ihdr_data = struct.pack(">IIBBBBB", width, height, 8,
                         6 if bpp >= 3 else (4 if bpp == 2 else 0),  # color type
                         0, 0, 0)
...
def _char_sets():
  base = set(map(chr, range(32, 127))) | set(EXTRA_CHARS) | set(RUSSIAN_ALPHABET)
  unifont = set(base)

  # Add full Cyrillic block to base charset for Inter font support
  cyrillic_range = set(chr(cp) for cp in range(0x0400, 0x0460))
  base.update(cyrillic_range)
...
def main():
  ...
  print(f"\nCharset summary:")
  print(f"  Base glyphs: {len(base_cp)}")
  print(f"  Unifont glyphs: {len(unifont_cp)}")

  for font in fonts:
    if "emoji" in font.name.lower():
      print(f"  Skipping emoji font: {font.name}")
      continue

    is_unifont = font.stem.lower().startswith("unifont")
    glyphs = unifont_cp if is_unifont else base_cp

    font_type = "unifont (full language support)" if is_unifont else "main font"
    print(f"\nProcessing {font.name} ({font_type}): {len(glyphs)} glyphs")
